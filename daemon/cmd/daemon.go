...
func (d *Daemon) AddHostLegacyRoutingRedirectPolicy(intfName string) error {
	...
	cmdChkStr := fmt.Sprintf("iptables -t mangle -C PREROUTING -i lxc+ -d %s -j MARK --set-mark %#08x/%#08x",
		nodeIPStr, linux_defaults.RouteMarkHostLegacyRoute, linux_defaults.RouteMaskHostLegacyRoute)
	cmd := exec.Command("bash", "-c", cmdChkStr)
	if _, err := cmd.CombinedOutput(); err != nil {
		cmdAddStr := fmt.Sprintf("iptables -t mangle -A PREROUTING -i lxc+ -d %s -j MARK --set-mark %#08x/%#08x",
			nodeIPStr, linux_defaults.RouteMarkHostLegacyRoute, linux_defaults.RouteMaskHostLegacyRoute)
		cmd := exec.Command("bash", "-c", cmdAddStr)
