...
type OpenStackAPI interface {
	...
	CreateNetworkInterface(ctx context.Context, subnetID, netID, instanceID string, groups []string, pool ipam.Pool, maxENICount int) (string, *eniTypes.ENI, error)
...
func (m *InstancesManager) CreateNode(obj *v2.CiliumNode, node *ipam.Node) ipam.NodeOperations {
	...
	m.instances.UpdateInstance(node.InstanceID(), nil)
